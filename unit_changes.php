    ...
    if(isset($_POST['edit_id'])) {
        ...
        if(!empty($edit_id)) {
            if(isset($_POST['unit_name'])) {
                ...
                $unit_name_error = $valid->common_validation($single_unit_name, "Unit Name", "1");
            }
        ...
        if(empty($edit_id)) {
            ...
            if(!empty($unit_name)) {
                for ($p = 0; $p < count($unit_name); $p++) {
                    $unit_name_error = $valid->common_validation($unit_name[$p], "Invalid Unit name - ". $unit_name[$p], "1");
                    if(!empty($unit_name_error)) {
