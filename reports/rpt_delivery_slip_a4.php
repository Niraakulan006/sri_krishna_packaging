<?php
    ...
    $view_delivery_slip_id = "";
    if (isset($_REQUEST['view_delivery_slip_id'])) {
        $view_delivery_slip_id = $_REQUEST['view_delivery_slip_id'];
    } else {
        header("Location: ../delivery_slip.php");
        exit;
    }
    
    if(isset($_REQUEST['view_delivery_slip_id'])) { 
        $view_delivery_slip_id = trim($_REQUEST['view_delivery_slip_id']);
        $bill_date = date('Y-m-d'); $delivery_slip_number = ""; $supplier_id = ""; $location_type = ""; $godown_type = "";$supplier_details ="";$product_count = 0; $godown_ids = array(); $factory_ids = array(); $size_ids = array(); $gsm_ids = array(); $godown_name = array(); $factory_name = array(); $size_name = array(); $gsm_name = array();$bf_name = array(); $quantity = array(); $selected_godown_id = ""; $selected_factory_id = "";$cancelled =0;
        $delivery_slip_list = array(); $godown_name_location = array();$remarks = "";
        $delivery_slip_list = $obj->getTableRecords($GLOBALS['delivery_slip_table'], 'delivery_slip_id', $view_delivery_slip_id);
        ...
        $pdf->SetY($bill_to_y);
        $pdf->SetX(110);
        $pdf->Cell(80,8,'Delivery Slip No   ',0,0,'L',0);

        $pdf->SetFont('Arial','',9);
        $pdf->SetX(142);
        $pdf->Cell(40,8,": ".$delivery_slip_number,0,1,'L',0);
        ...
        if (!empty($view_delivery_slip_id) && !empty($size_name)) {
            for ($p = 0; $p < count($size_name); $p++) {
                ...
                if ($pdf->GetY() >= 265) {
                    ...
                    $pdf->SetFont('Arial', 'B', 10);
                    // $pdf->SetY(5);
                    // $pdf->Cell(0, 5, 'Delivery Slip', 0, 0, 'C', 0);
                    $pdf->SetFont('Arial', 'BI', 10);
        ...
        $pdf->OutPut('', $delivery_slip_number);
    }
