<?php
    ...
    $view_inward_approval_id = "";
    if (isset($_REQUEST['view_inward_approval_id'])) {
        $view_inward_approval_id = $_REQUEST['view_inward_approval_id'];
    } else {
        header("Location: ../inward_approval.php");
        exit;
    }
    
    if(isset($_REQUEST['view_inward_approval_id'])) { 
        $view_inward_approval_id = trim($_REQUEST['view_inward_approval_id']);
        $bill_date = date('Y-m-d'); $inward_approval_number = ""; $supplier_id = ""; $location_type = ""; $godown_type = "";$supplier_details ="";$product_count = 0; $godown_ids = array(); $factory_ids = array(); $size_ids = array(); $gsm_ids = array(); $godown_name = array(); $factory_name = array(); $size_name = array(); $gsm_name = array();$bf_name = array(); $quantity = array(); $selected_godown_id = ""; $selected_factory_id = "";$cancelled =0;
        $inward_approval_list = array(); $godown_name_location = array();$remarks = "";
        $inward_approval_list = $obj->getTableRecords($GLOBALS['inward_approval_table'], 'inward_approval_id', $view_inward_approval_id);
        if(!empty($inward_approval_list)) {
            foreach($inward_approval_list as $data) {
                ...
                if(!empty($data['inward_approval_number']) && $data['inward_approval_number'] != $GLOBALS['null_value']) {
                    $inward_approval_number = $data['inward_approval_number'];
                }
        ...
        $pdf->SetX(10);
        $pdf->Cell(50,6,'Godown :  ',0,1,'L',0);

        $pdf->SetFont('Arial','',9);
        $pdf->SetX(30);
        $pdf->MultiCell(40,6,$godown_name_location,0,'L',0);
        $party_y = $pdf->GetY();
        $pdf->SetFont('Arial','B',9);
        $pdf->SetY($bill_to_y);
        $pdf->SetX(110);
        $pdf->Cell(80,8,'Inward Approval No   ',0,0,'L',0);

        $pdf->SetFont('Arial','',9);
        $pdf->SetX(142);
        $pdf->Cell(40,8,": ".$inward_approval_number,0,1,'L',0);
        ...
        if (!empty($view_inward_approval_id) && !empty($size_name)) {
            for ($p = 0; $p < count($size_name); $p++) {
                ...
                if ($pdf->GetY() >= 265) {
                    ...
                    $pdf->SetFont('Arial', 'B', 10);
                    // $pdf->SetY(5);
                    // $pdf->Cell(0, 5, 'Inward Approval', 0, 0, 'C', 0);
                    $pdf->SetFont('Arial', 'BI', 10);
        ...
        $pdf->OutPut('', $inward_approval_number);
    }
