        ...
        if(!empty($total_records_list)) {
            ...
            foreach($total_records_list as $key => $data) {
                ...
                if(!empty($data['size_id']) && $data['size_id'] != $GLOBALS['null_value']) {
                    $size_name = $obj->getTableColumnValue($GLOBALS['size_table'],'size_id',$data['size_id'],'size_name');
                    $pdf->SetX(30);
                    $pdf->Cell(40,6,html_entity_decode($obj->encode_decode('decrypt', $size_name)),1,0,'C',0);
                }
                else{
                    $pdf->SetX(30);
                ...
                if(!empty($data['gsm_id']) && $data['gsm_id'] != $GLOBALS['null_value']) {
                    $gsm_name = $obj->getTableColumnValue($GLOBALS['gsm_table'],'gsm_id',$data['gsm_id'],'gsm_name');
                    $pdf->SetX(70);
                    $pdf->Cell(40,6,html_entity_decode($obj->encode_decode('decrypt', $gsm_name)),1,0,'C',0);
                }
                else{
                    $pdf->SetX(70);
                    $pdf->Cell(40,6,' - ',1,0,'C',0);
                ...
                if(!empty($data['bf_id']) && $data['bf_id'] != $GLOBALS['null_value']) {
                    $bf_name = $obj->getTableColumnValue($GLOBALS['bf_table'],'bf_id',$data['bf_id'],'bf_name');
                    $pdf->SetX(110);
                    $pdf->Cell(40,6,html_entity_decode($obj->encode_decode('decrypt', $bf_name)),1,0,'C',0);
                }
                else{
                    $pdf->SetX(110);
                    $pdf->Cell(40,6,' - ',1,0,'C',0);
                ...
                if(!empty($data['outward_unit'])) {
                    $pdf->SetX(150);
                    $total_stock += $data['outward_unit'];
                    $pdf->Cell(50,6,$data['outward_unit'],1,1,'R',0);
