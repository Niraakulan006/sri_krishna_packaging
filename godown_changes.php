<?php
        ...
        <form class="poppins pd-20 redirection_form" name="godown_form" method="POST">
            ...
            <div class="row p-3">
                ...
                <div class="col-md-12 py-3 text-center" >
                    <button class="btn btn-danger submit_button" type="button" onClick="Javascript:SaveModalContent('godown_form', 'godown_changes.php', 'godown.php');">
                        Submit
                    </button>
                </div>
            </div>
    ...
    if(isset($_POST['godown_name'])) {	
        $godown_name = ""; $godown_name_error = ""; $incharge_name = ""; $incharge_name_error = ""; $user_id = "";$user_id_error = ""; 
        ...
        if(isset($_POST['user_id'])) {
            ...
            if(!empty($role_id_error)) {
                if(!empty($valid_godown)) {
                    $valid_godown = $valid_godown." ".$valid->error_display($form_name, "role_id", $role_id_error, 'select');
                }
                else {
                    $valid_godown = $valid->error_display($form_name, "role_id", $role_id_error, 'select');
                }
            }
        ...
        if(empty($valid_godown) && empty($count_error)) {
            ...
            if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                $godown_details = ""; $lower_case_name = ""; $lower_case_user_id = ""; $name_location = ""; $lowercase_incharge_name = "";$lowercase_name_location = "";
                $incharge_name_mobile = ""; $admin = 0; 
                ...
                $prev_godown_id = "";$godown_error = ""; $prev_user_id = ""; $user_error = ""; $str_user_id = "";$prev_user_godown_id = "";
                        ...
                        if(empty($prev_user_id)) {
                            if(empty($prev_user_mbl_godown_id)) {
                                if(empty($add_access_error)) {
                                    ...
                                    $columns = array('created_date_time', 'creator', 'creator_name', 'bill_company_id', 'godown_id', 'godown_name', 'lower_case_name', 'location', 'name_location', 'lowercase_name_location', 'incharge_name', 'mobile_number', 'user_id', 'password', 'lowercase_incharge_name', 'godown_details', 'role_id','deleted');

                                    $values = array("'".$created_date_time."'", "'".$creator."'", "'".$creator_name."'", "'".$bill_company_id."'", "'".$null_value."'", "'".$godown_name."'", "'".$lower_case_name."'", "'".$location."'", "'".$name_location."'", "'".$lowercase_name_location."'", "'".$incharge_name."'", "'".$mobile_number."'", "'".$user_id."'", "'".$password."'", "'".$lowercase_incharge_name."'", "'".$godown_details."'","'".$role_id."'", "'0'");
                                ...
                                if(preg_match("/^\d+$/", $godown_unique_id)) {
                                    if(empty($edit_access_error)) {
                                        ...
                                        if(preg_match("/^\d+$/", $godown_update_id)) {
                                            ...
                                            $columns = array(); $values = array();
                                            $columns = array('creator_name', 'name','role_id','login_id' ,'mobile_number', 'name_mobile', 'lower_case_login_id', 'password');
                
                                            $values = array("'".$creator_name."'", "'".$incharge_name."'", "'".$role_id."'", "'".$user_id."'","'".$mobile_number."'", "'".$incharge_name_mobile."'", "'".$lower_case_user_id."'", "'".$password."'");
        ...
        if(preg_match("/^\d+$/", $godown_unique_id)) {
            ...
            if(empty($delete_access_error)) {
                ...
                if(preg_match("/^\d+$/", $user_unique_id)) {
                    ...
                    $values = array("'1'");
                    $delete_user = $obj->UpdateSQL($GLOBALS['user_table'], $user_unique_id, $columns, $values, $action);
                }
                $columns = array(); $values = array();			
                $columns = array('deleted');
                $values = array("'1'");
