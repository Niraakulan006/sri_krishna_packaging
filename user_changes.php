...
?>
            ...
            <div class="row p-3">
                ...
                <div class="col-lg-3 col-md-4 col-12 py-2">
                    <div class="form-group">
                        ...
                        <input type="text" id="name" name="name" class="form-control shadow-none" value="<?php if(!empty($name)) { echo $name; } ?>" onkeydown="Javascript:KeyboardControls(this,'text',15,1);">
                ...
                <div class="col-lg-3 col-md-4 col-12 py-2">
                    <div class="form-group">
                        <div class="form-label-group in-border">
                        <input type="number" id="" name="mobile_number" class="form-control shadow-none" value="<?php if(!empty($mobile_number)) { echo $mobile_number; } ?>" onfocus="Javascript:KeyboardControls(this,'mobile_number',10,'');" onkeyup="Javascript:InputBoxColor(this,'text');">
                    ...
                    <div id="password_cover" class="form-group">
                        <div class="form-label-group in-border">
                            <div class="input-group">
                                <input type="password" class="form-control shadow-none" id="password" name="password" value="<?php if(!empty($password)) { echo $password; } ?>" placeholder="Password" onkeyup="Javascript:CheckPassword('password');" onfocus="Javascript:KeyboardControls(this,'password',20,'');" onkeydown="Javascript:InputBoxColor(this,'text');">
    ...
    if(isset($_POST['name'])) {	
        ...
        if(!empty($username) && strlen($username) > 25) {
            ...
            $username_error = $valid->valid_user_id($username,'user ID','1');
        }        
        ...
        if(empty($valid_user) && empty($access_permission_error)) {
            ...
            if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                if(!empty($edit_id)) {
                    $godown_id = $obj->getTableColumnValue($GLOBALS['user_table'], 'user_id', $edit_id, 'godown_id');
                    ...
                    if(!empty($godown_id) && $godown_id != $GLOBALS['null_value']) {
                        ...
                        $godown_details = $obj->getTableColumnValue($GLOBALS['godown_table'], 'id', $godown_unique_id, 'godown_details');
                    }
                        
                }
                   
                if(!empty($godown_details) && $godown_details != $GLOBALS['null_value']) {
                    $godown_details = $obj->encode_decode('decrypt', $godown_details);
                    $godown_details = explode("<br>", $godown_details);
                    if(strpos($godown_details[count($godown_details) - 1], "Incharge") !== false) {
                        $deleted_value = "";
                        $deleted_value = array_pop($godown_details);
                    }
                    $godown_details = implode("<br>",$godown_details);
                    $godown_details = $godown_details."<br>Incharge - ".$name_mobile;
                ...
                if(empty($edit_id)) {
                    if(empty($prev_user_id)) {
                        ...
                        $columns = array('created_date_time', 'creator', 'creator_name', 'user_id', 'name', 'mobile_number', 'name_mobile', 'password', 'admin', 'type', 'godown_id',  'role_id', 'login_id', 'lower_case_login_id','deleted');
                        $values = array("'".$created_date_time."'", "'".$creator."'", "'".$creator_name."'", "'".$null_value."'", "'".$name."'", "'".$mobile_number."'", "'".$name_mobile."'",  "'".$password."'", "'".$admin."'", "'".$type."'", "'".$null_value."'", "'".$role_id."'","'".$login_id."'", "'".$lower_case_login_id."'", "'0'");
                    ...
                    if(empty($prev_user_id) || $prev_user_id == $edit_id) {
                        ...
                        if(preg_match("/^\d+$/", $getUniqueID)) {
                            ...
                            $columns = array(); $values = array();						
                            $columns = array('creator_name', 'name', 'mobile_number', 'name_mobile', 'lower_case_login_id', 'password', 'role_id','login_id');
                            $values = array("'".$creator_name."'", "'".$name."'", "'".$mobile_number."'", "'".$name_mobile."'", "'".$lower_case_login_id."'", "'".$password."'" ,"'".$role_id."'","'".$login_id."'");
                ...
                if(!empty($show_records_list)) { 
                    foreach($show_records_list as $key => $data) {
                        ...
                        <tr>
                            ...
                            <?php 
                            if($data['godown_id'] == $GLOBALS['null_value']){ ?>
                                 <td>
                                    <?php
                                        $edit_access_error = "";
                                    ...
                                    <?php if (empty($edit_access_error) || empty($delete_access_error)) { ?>
                                        <div class="dropdown">
                                            ...
                                            $linked_count = 0;
                                            // $linked_count = $obj->GetUserLinkedCount($data['user_id']); 
                                            if (empty($delete_access_error) && $linked_count == 0) {
                                                if(empty($data['admin']) && $data['godown_id'] == $GLOBALS['null_value']){ ?>
