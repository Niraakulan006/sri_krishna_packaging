<?php
	include("sri_krishna_packaging_04082025.php");
	class Basic_Functions extends db {
		...
		public function getProjectTitle() {
			$project_title = "";
			if(empty($project_title)) { $project_title = "Sri Krishna Packaging";}
		...
		public function CompanyCount() {
			...
			$select_query = "SELECT COUNT(id) as factory_count FROM ".$GLOBALS['factory_table']." WHERE deleted = '0'";
			$list = $this->getQueryRecords($GLOBALS['factory_table'], $select_query);
			if(!empty($list)) {
				foreach($list as $data) {
					if(!empty($data['factory_count'])) {
						$count = $data['factory_count'];
						$count = trim($count);
					}
				}
			}
