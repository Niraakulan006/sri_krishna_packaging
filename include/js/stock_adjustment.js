		...
		url: post_url, success: function (check_login_session) {
			if (check_login_session == 1) {
                ...
                if(empty_row == "") {
                    ...
                    if(jQuery('select[name="selected_stock_type"]').is(":visible")) {
                        if(jQuery('select[name="selected_stock_type"]').length > 0) {
                            selected_stock_type = jQuery('select[name="selected_stock_type"]').val().trim();
                            if(selected_stock_type == "" || selected_stock_type == null || typeof selected_stock_type == "undefined") {
                                all_errors_check = 0;
                                if(jQuery('select[name="selected_stock_type"]').parent().parent().parent().find('span.infos').length == 0) {
                                    jQuery('select[name="selected_stock_type"]').parent().parent().append('<span class="infos text-danger"><i class="fa fa-exclamation-circle"></i>Select Stock Type</span>');
                                }
                            }
                            else {
                                if(jQuery('select[name="selected_stock_type"]').parent().parent().parent().find('span.infos').length > 0) {
                                    jQuery('select[name="selected_stock_type"]').parent().parent().parent().find('span.infos').remove();
                                }
                            }
                        }
                    }
                    ...
                    if(jQuery('.product_table').find('tr.product_row').length > 0) {
                        jQuery('.product_table').find('tr.product_row').each(function() {
                            ...
                            if(jQuery(this).find('select[name="stock_type[]"]').length > 0) {
                                prev_stock_type = jQuery(this).find('select[name="stock_type[]"]').val().trim();
                            }
                            if(parseInt(location_type) == 1) {
                    ...
                    if(error == "") {
                        ...
                        jQuery.ajax({
                            url: post_url, success: function (result) {
                                ...
                                if(jQuery('select[name="selected_stock_type"]').length > 0) {
                                    jQuery('select[name="selected_stock_type"]').val('').trigger('change');
                                }
                                if(jQuery('select[name="location_type"]').length > 0) {
...
function AdjustmentRowCheck(obj) {
